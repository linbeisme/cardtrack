...
function promptManagerHtml() {
  ...
  const usage = state.promptWorkflow === "two-step"
    ? (state.promptStage === "json" ? "Paste this into the same Deep Research conversation after Step 1 finishes. Copy the returned JSON code block with its Copy button, then paste it below." : "Start a new Deep Research conversation with this prompt. When the evidence ledger finishes, return here and copy Step 2.")
    : `Paste this into a normal ${state.promptProvider === "gemini" ? "Gemini chat with Google Search" : "ChatGPT chat with Search enabled"}; do not select Deep Research. Copy the returned JSON code block with its Copy button.`;
  const preflightItems = [
    ["Catalog", preflight.hasCatalog], ["Schema", preflight.hasSchema], ["Source policy", preflight.hasSourcePolicy],
...
async function handleAction(event) {
  ...
  try {
    ...
    } else if (action === "copy-repair-prompt") {
      if (state.promptDirty) saveCurrentEditorToState(false);
      const template = state.stagedPrompts.templates.find((item) => item.id === state.selectedTemplateId);
      const repairPrompt = resolveRepairPromptVariant(state.stagedPrompts, template, state.stagedDatabase.cards, new Date(), state.stagedDatabase, {provider: state.promptProvider});
      await navigator.clipboard.writeText(repairPrompt);
      toast(`${state.promptProvider === "gemini" ? "Gemini" : "ChatGPT"} JSON repair prompt copied.`);
