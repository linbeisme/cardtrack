...
async function handleAction(event) {
  ...
  try {
    if (action === "toggle-theme") {
      ...
      state.stagedDatabase.updatedBy = "cardtrack-admin-publisher-v4.1.1";
...
async function runGithub(mode) {
  ...
  try {
    if (mode === "test") {
      ...
      const result = await putJsonFile({...base, path: state.repository.dataPath, value: state.stagedDatabase, message: "Update CardTrack database from Admin Publisher v4.1.1"});
